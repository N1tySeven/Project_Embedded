...
## 🔄 ลำดับการทำงานของระบบ (System Workflow)
1. **เริ่มต้นระบบ (Boot up):** 
   - ระบบเคลียร์หน้าจอ เริ่มต้นโมดูลทั้งหมด (LCD, Keypad, SPI, SD Card, RFID)
   - สั่งล็อคประตูอัตโนมัติ (Servo หมุนปิด) 
   - รอผู้ใช้งานนำบัตรมาสแกน ("Scan Card / Ready...")
2. **กรณีบัตรเคยลงทะเบียนไว้แล้ว (Registered Card):**
   - ระบบจะให้กรอกรหัส **PIN 4 หลัก** ผ่าน Keypad
   - หากรหัสถูกต้อง: ประตูจะปลดล็อค และเข้าสู่เมนูหลัก (Menu)
     - `กด 1:` ปลดล็อคประตูด้วยมือ (Manual Unlock)
     - `กด 2:` เปลี่ยนรหัส PIN ของบัตรใบนั้น (ระบบจะบันทึกลง SD Card ทันที)
     - `กด 3:` ออกจากระบบ (Logout) และล็อคประตู
   - หากรหัสผิด: ระบบแจ้งเตือน Invalid PIN และกลับไปหน้าสแกนบัตร
3. **กรณีบัตรยังไม่เคยลงทะเบียน (Unregistered Card):**
   - ระบบจะขอรหัสผ่านแอดมิน (Admin Password) ค่าเริ่มต้นคือ `1234`
   - หากใส่รหัสแอดมินถูกต้อง: ระบบจะให้ผู้ใช้ตั้ง **รหัส PIN ใหม่ 4 หลัก** สำหรับบัตรใบนั้น
   - บันทึก UID ของบัตรคู่กับรหัส PIN ลงในไฟล์ `uid.txt` บน SD Card ทันที
